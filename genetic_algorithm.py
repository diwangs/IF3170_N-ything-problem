	...
	def crossover(self, board1, board2):
		...
		for i in range(new_board1.size):
			if (mask & (1 << i)):
				...
				new_board1.add_piece(pickle.loads(pickle.dumps((board2.pieces[i]))))
				new_board2.add_piece(pickle.loads(pickle.dumps((board1.pieces[i]))))
				...
				new_board1.add_piece(pickle.loads(pickle.dumps((board1.pieces[i]))))
				new_board2.add_piece(pickle.loads(pickle.dumps((board2.pieces[i]))))
				# set new board reference
				new_board1.pieces[-1].board = new_board1
				new_board2.pieces[-1].board = new_board2
